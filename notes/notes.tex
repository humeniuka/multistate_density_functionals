...
% Motivation
Kohn-Sham (KS) density functional theory (DFT) is probably the single most used electronic structure
...
For molecules, linear-response time-dependent DFT (LR-TDDFT) provides excited state properties.
However, both methods fail when an excited state becomes nearly degenerate with the ground state,
which cannot be described by a single Slater determinant. This happens at unstable molecular
geometries, when bonds are broken, at transition states and conical intersections.
...
The development of a multi-configurational and variational DFT is an active research area.
...
guaranteed by the HK theorem) but that their
...
% How do the lowest excited states of the homogeneous electron look like, anyway?
%
...
% [The Lu & Gao theorems only work for the discrete part of the spectrum, for bound states of finite systems, where a matrix representation is adequate]
...
% The trace of the effective Hamiltonian is not invariant 
...
% Another advantage of MSDFT over ensemble DFT \cite[4.4.4 Maitra and Ullrich]{Teale2022dft_exchange}
% is that it gives the oscillator strengths, since the matrix density contains the transition densities.
%
...
\end{equation}
However, deducing the exact multi-state functional from known ground-state kinetic-energy and Hartree-exchange-correlation functionals is not possible. Even if one has a closed expression of the functionals in term of $\rho(\vec{r})$, $\nabla \rho(\vec{r})$, $\ldots$, it is not clear where $\rho(\vec{r})$ has to be replaced by the scalar $\tr(\m{D}(\vec{r}))$ and where by the matrix $\m{D}(\vec{r})$. For $N=1$, all three are the same, i.e. $\rho(\vec{r}) = \tr(\m{D}(\vec{r})) = \m{D}(\vec{r})$.
...
So one has to find a different generalization of the von-Weizs\"{a}cker functional to multiple electronic states. A ground state functional can be turned in multistate functional by reusing its functional form and simply replacing occurances of the density $\rho(\vec{r})$ by the matrix density $\m{D}(\vec{r})$.
For the von-Weizs\"{a}cker functional this ad-hoc prescription gives,
\begin{equation}
...
For the small systems studied, equations \ref{eqn:vW_ms_extension} and \ref{eqn:vW_ms_extension_matrix_square_root} give slightly different results, but it is not clear which one is superior.
...
\begin{figure}[h!]
\includegraphics[width=0.8\textwidth]{figures/vW_kinetic_energy_LiH-lsda.png}
...
% The factor 1/9 in T[rho] = T_TF[rho] + 1/9 T_vW[rho] 
% is explained in section 6.7 "Conventional gradient correction"
% in Parr & Yang's book.      
...
\begin{figure}[h!]
  \includegraphics[width=0.9\textwidth]{figures/lithium-fluoride_electron_repulsion_and_kinetic_energies-lsda.png}
\caption{
\textbf{Lithium-fluoride.}
...
\input{tables/lithium-fluoride_electron_repulsion_and_kinetic-lsda.tex}
...
\section{Exchange-correlation energy density}

For the ground state density $\rho(\vec{r})$ the exchange-correlation \emph{energy density} can be defined as
\begin{equation}
 \varepsilon^{xc}(\vec{r}) = \frac{1}{2} \int d^3r \frac{P(\vec{r},\vec{r}') - \rho(\vec{r}) \rho(\vec{r}')}{\vert \vec{r} - \vec{r}' \vert}.
\end{equation}
This definition is not unique, as any function that integrates to the correct exchange-correlation \emph{energy} is acceptable.
In the multistate case, the 1-electron matrix density is defined as
\begin{equation}
D_{IJ}(\vec{r}) = n \int d^3r_2 \int d^3r_3 \cdots \int d^3r_n
  \Psi^*_I(\vec{r},\vec{r}_2,\ldots,\vec{r}_n)
  \Psi_J(\vec{r},\vec{r}_2,\ldots,\vec{r}_n)
\end{equation}
and the particle (or 2-electron) matrix density is
\begin{equation}
D^{2e}_{IJ}(\vec{r},\vec{r}') = n (n-1) \int d^3r_3 \cdots \int d^3r_n 
  \Psi^*_I(\vec{r},\vec{r}',\vec{r}_3,\ldots,\vec{r}_n)
  \Psi_J(\vec{r},\vec{r}',\vec{r}_3,\ldots,\vec{r}_n)
\end{equation}
where $n$ is the total number of electrons.
The multistate matrix for the exchange-correlation energy density then becomes
\begin{equation}
\textbf{XC}_{IJ}(\vec{r}) =  \frac{1}{2} \int d^3r' \frac{
  D^{2e}_{IJ}(\vec{r},\vec{r}') - \sum_K D_{IK}(\vec{r}) D_{KJ}(\vec{r}')}{\vert \vec{r} - \vec{r}' \vert}.
\end{equation}
It is important that the product of the 1-particle densities $\rho(\vec{r}') \rho(\vec{r})$ is translated
into a matrix product of the 1-partice matrix densities, $\sum_K D_{IK}(\vec{r}) D_{KJ}(\vec{r}')$, to ensure
that the 2-particle matrix density is equivariant under basis transformations in the electronic space.

When the pair density is expressed in a basis of atomic orbitals $\chi_{p}(\vec{r})$,
\begin{equation}
D^{2e}_{IJ}(\vec{r},\vec{r}') = \sum_{p,q,r,s}^{\text{AOs}} D^{2e}_{IJ,pqrs} 
  \chi_p(\vec{r}) \chi_q(\vec{r}) \chi_r(\vec{r}') \chi_s(\vec{r}'),
\end{equation}

the matrix for the exchange-correlation energy density becomes

\begin{equation}
  \textbf{XC}_{IJ}(\vec{r}) = \frac{1}{2} \sum_{p,q,r,s}
  \left( 
    D^{2e}_{IJ,pqrs} - \sum_K D_{IK,pq} D_{KJ,rs}
  \right) \chi_p(\vec{r}) \chi_q(\vec{r}) V_{rs}(\vec{r})
\end{equation}
where
\begin{equation}
  V_{rs}(\vec{r}) = \int \frac{\chi_r(\vec{r}') \chi_s(\vec{r}')}{\vert \vec{r} - \vec{r}' \vert}
\end{equation}
is the Coulomb potential generated by the charge distribution $\chi_r(\vec{r}) \chi_s(\vec{r})$,
which can be calculated using integral routines developed for the nuclear attraction integrals.

\begin{figure}
  % plotted by multistate_density_functionals/examples/16-exchange-correlation-energy-density/plot_xc_energy_density.py
\includegraphics[width=0.7\textwidth]{figures/hydrogen-molecule_xc_energy_density.png}
\caption{Exchange-correlation energy density XC$_{IJ}(\vec{r})$ for the hydrogen molecule computed at the FCI/6-31g level of
theory for the lowest 3 singlet states.
}
\end{figure}
