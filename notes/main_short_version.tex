...
\myexternaldocument{supporting_information_short_version}
...
In wavefunction theory (WFT) near-degenerate states can be included in a multi-state treatment
...
and a \emph{transition density functional} (TDF), \cite[Eqn.~(5)]{Grofe2017} has been
...
states in a multiplet (same total spin but different spin projections) are degenerate in the absence of a magnetic field.
These approximations make use of existing, Kohn-Sham density functional approximations (DFA)
for the ground state and are born out by encouraging benchmark calculations \cite{Zhu2023}.
But in view of Lu and Gao's multistate equivalent of the Hohenberg-Kohn theorem,
...
However, because all the complexity is packed into one
...
MSDFT resembles the state-averaged MCSCF of wavefunction theory,
...
The approximate functional is tested on the avoided crossing in LiF (section \ref{sec:results}) and section \ref{sec:conclusion} concludes.

Proofs of the lower bounds on the average kinetic and electron-repulsion energies in terms of the subspace density are relegated to section
\ref{sec:lower_bounds} of the supporting information.
...
\begin{equation}
 ...
 \end{equation}
 This multi-state analogue of the Hohenberg-Kohn theorem states that the Hamiltonian can be split into an
external potential (the attractive nuclear potential $v(\vec{r})$ which differs for each molecule)
and a universal functional $\m{\mathcal{F}}[\m{D}(\vec{r})]$ for the
...
% This multi-state analogue of the Hohenberg-Kohn theorem states that the Hamiltonian can be split into an
% external potential (the attractive nuclear potential $v(\vec{r})$ which differs for each molecule)
% and a universal functional $\m{F}[\m{D}(\vec{r})]$ for the
% electronic kinetic energy and electron-electron repulsion (that is the same for all systems).
% \begin{equation}
%  \m{H}[\m{D}(\vec{r})] = \m{\mathcal{F}}[\m{D}(\vec{r})] + \int d^3\!r ~ v(\vec{r}) \m{D}(\vec{r})
% \end{equation}
...
An important consequence of the fact that both $\m{D}(\vec{r})$ and $\m{H}[\m{D}(\vec{r})]$
are projections of operators onto the same subspace, is
...
the matrix density in the new basis becomes $\m{D}'(\vec{r}) = \m{L} \m{D}(\vec{r}) \m{L}^{\dagger}$, and the Hamiltonian becomes 
$\m{H}' = \m{L} \m{H} \m{L}^{\dagger}$.
Consequently the universal functional has to transform as \cite{Lu2022JPCL}
\begin{equation}
 \m{\mathcal{F}}[\m{L} \m{D}(\vec{r}) \m{L}^{\dagger}] = \m{L} \m{\mathcal{F}}[\m{D}(\vec{r})] \m{L}^{\dagger} \label{eqn:F_basis_transformation}
\end{equation}
under a basis transformation $\m{L}$ with $\m{L}^{-1} = \m{L}^{\dagger}$. Note that $\m{D}(\vec{r})$ is an $N \times N$ matrix function of $\vec{r}$,
...
The significance of the theorems by Lu and Gao is not so much the existence of the mapping $\m{D}(\vec{r}) \to \m{H}$ (which is already
...
It constrains the search for matrix functionals to the class of \emph{analytic matrix functionals}, as will be explained later on.
It also implies that its functional form
is independent of the number of states in the subspace, so that the same functional holds for $N$ states or for a single state $1$.
...
\end{align}
The exchange-correlation energy density $\epsilon_{xc}(\rho)$ and the kinetic energy density $t(\rho)$ are functions
...
of the matrix density $\m{D}$, by taking the coefficients of the Taylor series and replacing $\rho$ by $\m{D}$,
\begin{equation}
...
The corresponding LDA matrix functional $\m{\mathcal{F}}[\m{D}]$ becomes
\begin{equation}
  \m{\mathcal{F}}[\m{D}(\vec{r})] = \int d^3\!r ~ \m{f}(\m{D}(\vec{r})).
\end{equation}
...
If the scalar functional $\mathcal{F}[\rho]$ only depends on the total electronic densities (summed over spins), as in the LDA,
this procedure determines a unique matrix functional $\m{\mathcal{F}}[\m{D}]$.
...
or the von-Weizs\"{a}cker (vW) kinetic energy density $t_{\text{vW}}(\rho(\vec{r}),\nabla \rho(\vec{r}))$ are functions of both the density and its gradient.
...
\section{Theory}
In the following the subscripts $a,b = 1,\ldots,n$ enumerate electrons, Latin captial letters $I,J,K,L = 1,\ldots,N$
enumerate electronic states in the subspace.
...
The transformation property \ref{eqn:F_basis_transformation} implies that \textbf{$\mathcal{F}$ is an \emph{analytic matrix functional}}.
As we know, an analytic function of a scalar variable, $f(z): \mathbb{C} \to \mathbb{C}$ is defined by its Taylor series around some point $z_0$:
\begin{equation}
...
\begin{equation}
  f(\m{L} \m{Z} \m{L}^{-1}) = \sum_{k=0}^{\infty} \frac{f^{(k)}}{k!} (\m{L} \m{Z} \m{L}^{-1})^k = \m{L} f(\m{Z}) \m{L}^{-1},
\end{equation}
since $(\m{L} \m{Z} \m{L}^{-1})^k = \m{L} \m{Z} (\m{L}^{-1} \m{L}) \m{Z} \m{L}^{-1} \cdots = \m{L} \m{Z}^k \m{L}^{-1}$.
...
Because of the transformation property \ref{eqn:F_basis_transformation} the series expansion of the matrix density functional
...
Let $v(r)$ be the external potential (for instance the Coulomb attraction to the nuclei in a molecule) that is localized around the origin $\vec{O}$. $v(r)$ decays like $\propto -\frac{1}{r}$ or faster with distance. A copy of the same nuclei is put at a distant point $\vec{R}$,
so that the total external potential is $v(\vec{r}) + v(\vec{r}-\vec{R})$
as shown in Fig.~\ref{fig:repeated_effective_potential} of the SI.
...
These wavefunctions decay exponentially with distance \cite{Katriel1980asymptotic}.
$\mathbb{V}_{\vec{R}}^N$ is spanned by the same wavefunctions rigidly translated to the center $\vec{R}$.
A subspace of twice the dimension is formed by combining the subspaces of the left and right molecules,
$\mathbb{V}^{2N} = \mathbb{V}^{N}_{\vec{O}} \cup \mathbb{V}^{N}_{\vec{R}}$.
% \begin{equation}
%   \begin{split}
...
%   \end{split}
% \end{equation}
Because of the large distance $R$, the transition densities between states localized at $\vec{O}$ and those at $\vec{R}$ vanish.
The state densities and transition densities among the states localized on the same molecule are the same
for both moleules except for a rigid shift. The matrix density for the combined subspace therefore consists of two blocks of size $N$, each,
...
By using the series expansion \ref{eqn:functional_taylor_expansion} for $\m{\mathcal{F}}^{(2N)}[\m{D}^{(2N)}(\vec{r})]$,
splitting the integration volume into the Voronoi polyhedra, using relations \ref{eqn:D_split} and \ref{eqn:N_split}
and the translational invariance,
it can be shown (see Eqn.~\ref{eqn:relation_F2N_FN_same_dim} in the SI for details) that
$\m{\mathcal{F}}^{(2N)}[\m{D}^{(2N)}(\vec{r})] = \m{\mathcal{F}}^{(2N)}[\m{D}^{(N)}(\vec{r})] \oplus \m{\mathcal{F}}^{(2N)}[\m{D}^{(N)}(\vec{r})]$. From comparison with Eqn.~\ref{eqn:relation_F2N_FN} it then follows that
\begin{equation}
...
% \end{equation}
% for any matrix density $\m{D}^{(N)}(\vec{r})$ that decays exponentially with distance.
Now the dimension of the argument is the same on both sides. The above equality suggests that the same universal functional applies to all subspace dimensions and one can drop the superscript and simply write $\m{\mathcal{F}}[\m{D}(\vec{r})]$. For $N=1$ it is equal to the ground-state kinetic and Hartree-exchange-correlation (Hxc) functionals,
\begin{equation}
...
\end{equation}
the functionals for $\hat{T}$ and $\hat{W}$ have to be approximated. The kinetic energy multistate functional $\m{T}[\m{D}]$
will be derived from the Thomas-Fermi and von-Weizs\"{a}cker ground state functionals. The electron-electron repulsion
is split into a Hartree-like direct term $\m{J}[\m{D}]$, a Dirac-like exchange term $-\m{K}^{\text{LDA}}[\m{D}]$,
a correlation-like term $\m{C}^{\text{LDA}}[\m{D}]$ based on Chachiyo's fit to
the correlation energy of the homogeneous electron gas and a self-interaction correction for the core electrons.
Each functional of the matrix density behaves properly under basis transformations of the state vectors.
Auxiliary basis states are not used to compute any parts of the Hamiltonian.
Let us start with the electron-electron repulsion.
...
The exchange-like term in the local density approximation (LDA) \cite[6.1.20]{ParrYangBook} \cite{Dirac1930},
\begin{equation}
  \m{K}^{\text{LDA}}[\m{D}] = C_x \int (\m{D}(\vec{r}))^{4/3} ~ d^3r, \quad \quad C_x = 0.7386 \label{eqn:exchange_term_lda}
\end{equation}
...
the transition densities usually have a large spin-polarization. Unfortunately, it is not obvious how to turn the spin
...
we incur an error of approximately 10\%.
...
$\m{J}[\m{L} \m{D}(\vec{r}) \m{L}^{-1}] = \m{L} \m{J}[\m{D}(\vec{r})] \m{L}^{-1}$ under a basis transformation of the electronic states.
...
In particular neither functional is exact for 1-electron systems. A multistate von-Weizs\"{a}cker functional that is exact for
1-electron matrix densities is presented in section \ref{sec:exact_constraints} of the SI.

The kinetic functional that will be used in the Results section is taken as
\begin{equation}
\m{T}[\m{D}(\vec{r})] = \m{T}_{\text{TF}}[\m{D}(\vec{r})] + \frac{1}{9} \m{T}_{\text{vW}}[\m{D}(\vec{r})]. \label{eqn:kinetic_energy_msdft}
\end{equation}
...
% the individual matrix elements $\mathcal{F}_{IJ}[\m{D}]$.

% % \subsubsection{Lower Bound of Kinetic Energy}
% The kinetic energy of a single wavefunction is bounded from below by the von-Weiz\"{a}cker kinetic energy functional. \cite{Lieb2002}
% %The proof for this inequality can be found below theorem 1.1 of Ref.~\cite{Lieb2002}.
% Applying this bound individually
...
% The derivation proceeds in analogy with theorem 1.1 of Ref.~\cite{Lieb2002} (see SI) and leads to,
% % It is assumed that the $n$-electron wavefunctions are antisymmetric and orthonormal.
...
% \end{equation}
% This lower bound is a functional of the subspace density $\rho_{V} = \frac{1}{N} \tr(\m{D})$
% and is therefore an invariant property of the subspace.
...
orbital-free DFT \cite{Teale2022dft_exchange}.
...
The LDA-like multistate functional was tested on the dissociation curve of LiF.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{suppinfo}
properties of matrix density functionals,
comparison of paramagnetic and ferromagnetic correlation energy,
proof of Klein's trace inequality,
incorporation of exact constraints into von-Weizs\"{a}cker functional,
Lieb-Oxford-like lower bounds for kinetic and electron repulsion energy of subspace.
