...
\begin{equation}
 W^{\text{exact}}_{IJ} = E^{\text{exact}}_{I} \delta_{IJ} - \m{T} - \int v(\vec{r}) \m{D}(\vec{r})
\end{equation}
...
%\section{Discussion}
%\label{sec:dicussion}
A local functional of the matrix density reproduces the effective Hamiltonian in the subspace of the lowest electronic states.
...
The multistate functional was tested on the dissociation curve of LiF.
The exchange part of the functional makes the largest contribution to the state interactions,
while the correlation part is an order of magnitude smaller.
...
A limitation of this work is that the matrix functional was not determined by minimizing the subspace energy.
Using an efficient parameterization of the matrix density \cite[theorem (3)]{Lu2022JPCL} \cite{Lu2022fundamental},
multi-configurational self-consistent field calculations with an effective Hamiltonian computed from the mulistate functional
...
the multistate DFT of Lu and Gao would open the way for studying excitations in very large systems
