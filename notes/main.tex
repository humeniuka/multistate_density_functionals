...
Slater determinants built from $N^2$ different sets of $n_e$ molecular orbitals. \cite{Lu2022JPCL}
...
\begin{equation}
 \text{XC}[\m{D}(\vec{r})]_{IJ} = \langle \Psi_I^{\text{exact}} \vert \sum_{m < n} 1/r_{mn} \vert \Psi_J^{\text{exact}} \rangle - J[\m{D}(\vec{r})]_{IJ}.
\end{equation}
...
% Maybe the following should be moved into the computational section?
