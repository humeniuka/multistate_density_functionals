...
This is the first attempt to find an approximation to the multistate universal functional $\mathcal{F}[\m{D}(\vec{r})]$.
...
The translation property does not exclude the possibility that the functional depends implicitly on the dimension of the subspace, which can be expressed as $\mathcal{N}(\vec{r}) = \tr\{\m{D}(\vec{r}) \m{D}^{-1}(\vec{r})\}$ and therefore is also a functional of the matrix density.
