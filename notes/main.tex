...
% For referring to labels in supporting_information.tex
\makeatletter
...
\myexternaldocument{supporting_information}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\abbreviations{}
\keywords{excited states, time-independent density functional theory, multiconfigurational DFT}
...
are a universal functional of the matrix density.
This is the first attempt to find an approximation to the multistate universal functional $\mathcal{F}$.
It is shown that $\mathcal{F}$ (i) does not explicitly depend on the number of electronic states and (ii)
is an analytical matrix functional. 
...
properly under basis set transformations, and that the ground state functional should be recovered for a single state.
...
reproduces the matrix elements of the Hamiltonian in the basis of the exact eigenstates accurately for all bond lengths.
...
More exact conditions that constrain the functional form of $\mathcal{F}$ are needed to go beyond the local density approximation.
...
% * F is an analytical matrix functional.
...
but it has problems with core-excitations [CITATION],
...
Although some of these issues can be fixed with tuned \cite{Stein2009reliable} range-separated hybrid \cite{Tawada2004long} functionals or spin-flip DFT \cite{Shao2003spin},
...
The rigorous, multistate DFT theory by Lu and Gao \cite{Lu2022JPCL} might provide answers to those questions.
...
Section \ref{sec:properties_analytic_matrix_functional} introduces the term \emph{analytical matrix functional} to characterize $\m{\mathcal{F}}$
...
In section \label{sec:approximate_lda_msdft} a local density approximation to the universal functional is presented.
Section \label{sec:exact_constraints} discusses incorporating exact conditions to constrain the multistate matrix functional.
Section \label{sec:heg_msdft} explores whether there is a many-electron system for which the LDA multistate functional is exact.
Lieb-Oxford-style lower bounds on the kinetic and electron repulsion energies are formulated in section \label{sec:lower_bounds}.
The functional is tested on the avoided crossing in LiF (section \label{sec:results}).
%\label{sec:dicussion}
...
and excited states are treated on an equal footing. The fundamental variable is the projection
of the one-particle density operator $\hat{\rho}(\vec{r}) = \sum_{a=1}^{n_{\text{el}}} \delta(\hat{\vec{r}}_a - \vec{r})$ on the subspace
...
$D_{IJ}(\vec{r}) = \langle \Psi_I \vert \sum_{a=1}^{n_{\text{el}}} \delta(\hat{\vec{r}}_a - \vec{r}) \vert \Psi_J \rangle$ on the off-diagonal.
...
Slater determinants built from $N^2$ different sets of $n_{\text{el}}$ molecular orbitals. \cite{Lu2022JPCL}
...
\end{equation}
However, deducing the exact multi-state functional from known ground-state kinetic-energy and Hartree-exchange-correlation functionals is not possible.
...
the correlation energy of the homogeneous electron gas and a self-interaction correction. Let us start
...
Since $W_{IJ}[\m{D}]$ is an \emph{analytical} matrix functional,
...
\end{equation}
Because of the requirement that $W[\m{D}]_{IJ}$ is an analytical matrix functional, the Hartree-like term 
\begin{equation}
...
\subsubsection{Self-Interaction Error}
...
the MSDFT approximation $\m{J}[\m{D}]-\m{K}^{\text{LSDA}}[\m{D}^{\alpha},\m{D}^{\beta}] + \m{C}^{\text{LDA}}[\m{D}]$ does not depend on the geometry.
...
For the small systems studied, equations \ref{eqn:vW_ms_extension} and \ref{eqn:vW_ms_extension_matrix_square_root}
...
\begin{equation}
T[\m{D}(\vec{r})] = T_{\text{TF}}[\m{D}(\vec{r})] + \frac{1}{9} T_{\text{vW}}[\m{D}(\vec{r})]. \label{eqn:electron_repulsion_msdft}
\end{equation}
...
For a random set of electronic states there is no guarantee that $\m{H}[\m{D}]$ maps their matrix density to the corresponding Hamiltonian matrix.
...
and the kinetic energy $\m{T}[\m{D}]$ (Eqn.~\ref{eqn:electron_repulsion_msdft}) into the subspace of the lowest eigenstates are then compared with the
...
\input{tables/lithium-fluoride_electron_repulsion_and_kinetic-lda.tex}
...
A.H. thanks Prof. Toshinori Suzuki for the kind support.
%Calculations were performed with the supercomputer system of the Academic Center for Computing and Media Studies at Kyoto University.
\end{acknowledgement}
