...
\title{\centering Research Plan:\\ A New, Variational, Multistate Density Functional Theory for Predicting Optical Properties of Materials}
\date{\vspace{-3ex}}
...
Especially the kinetic functional, for which there is no good (semilocal) approximations, could benefit from machine learning (see Fig. \ref{fig:equivariant_ml_matrix_functional}).

\begin{figure}[h!]
    \centering
    \includegraphics[width=0.7\linewidth]{figures/equivariant_ml_matrix_functional.png}
    \caption{\label{fig:equivariant_ml_matrix_functional} Equivariant ML model for the kinetic energy functional.
    The same model accepts matrix densities with different numbers of states.}
\end{figure}
